(function($){window.console=window.console||(function(){var c={};c.log=c.warn=c.debug=c.info=c.error=c.time=c.dir=c.profile=c.clear=c.exception=c.trace=c.assert=function(){};return c})();$.fn.extend($.easing,{def:"easeInOutExpo",easeInOutExpo:function(e,f,a,h,g){if(f===0){return a}if(f===g){return a+h}if((f/=g/2)<1){return h/2*Math.pow(2,10*(f-1))+a}return h/2*(-Math.pow(2,-10*--f)+2)+a}});$(document).keydown(function(e){var keycode=e.keyCode;var $current_gridder=$(".currentGridder");var $current_target=$current_gridder.find(".gridder-show");if($current_gridder.length){if(keycode===37){$current_target.prev().prev().trigger("click");e.preventDefault()}
    ...
    _this.removeClass("hasSelectedItem");visible=!1;base.find(".selectedItem").removeClass("selectedItem");base.find(".gridder-show").slideUp(settings.animationSpeed,settings.animationEasing,function(){$('.gridder').find(".gridder-show").remove();settings.onClosed(base)});$(".currentGridder").removeClass("currentGridder")}
    function openExpander(myself){$(".currentGridder").removeClass("currentGridder");_this.addClass("currentGridder");if(!myself.hasClass("selectedItem")){_this.find(".selectedItem").removeClass("selectedItem");myself.addClass("selectedItem")}else{closeExpander(_this,settings)}
        $('.gridder').find(".gridder-show").remove();if(!_this.hasClass("hasSelectedItem")){_this.addClass("hasSelectedItem")}
