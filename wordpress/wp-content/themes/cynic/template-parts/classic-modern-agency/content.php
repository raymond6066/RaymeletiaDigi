...
<?php } ?>
<div class="blog-item-data"> <a href="<?php the_permalink()?>"><i class="icon-calendar-full"></i> <?php echo get_the_date().' '.get_the_time() ?></a> <span class="separator">&nbsp;</span> <?php if(has_category()){?><span class="author"><i class="icon-list4"></i> <?php echo get_the_category_list(esc_html__(', ', 'cynic'));?></span> <span class="separator">&nbsp;</span> <?php }?><a href="<?php the_author_meta('url')?>"><i class="icon-user"></i> <?php the_author();?></a> <span class="separator">&nbsp;</span> <a class="comments_link" href="<?php echo esc_url(get_comments_link(get_the_ID()))?>"><?php printf(_nx('%2$s Comments (1)', '%2$s Comments', get_comments_number(), 'comments title', 'cynic'), number_format_i18n(get_comments_number()), '<i class="icon-bubbles"></i>');?></a></div>
<div class="<?php echo esc_attr($class) ?> blog-media post-format-<?php echo get_post_format() ?>">
