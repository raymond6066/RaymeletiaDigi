...
if (!function_exists('cynic_seo_agency_comments')) {
    function cynic_seo_agency_comments($user_identity = null)
    {
        ...
        <div id="comments" class="comments-area">
            ...
            $fields = array(
                'author' => '<div class="form-group col-md-6"><input type="text" class="form-control" name="author" placeholder="' . __('Full Name', 'cynic') . '"></div>',
                'email' => '<div class="form-group col-md-6"><input type="email" name="email" class="form-control" placeholder="' . __('Email', 'cynic') . '"></div>'
            );

            $comments_args = array(
                'comment_field' => '<div class="form-group col-md-12"><textarea name="comment" class="form-control" id="comment-textarea" rows="3" placeholder="' . __("Message", "cynic") . '"></textarea></div>',
                'fields' => $fields,
                ...
                'submit_button' => '<div class="col-md-12"><footer class="form-footer"><button type="submit" class="primary-btn">' . __('Submit Comment', 'cynic') . '</button><div class="submit-loader"><img class="submit-loading-img" src="' . CYNIC_THEME_URI . '/images/submit-loader.gif" alt="submit loader"></div></div>',
            );
            ?>
    ...
    if (getCynicOptionsVal('theme_mode') == 2) {
        if ($blogType == 'single') {
            if (getCynicOptionsVal('blog_single_sidebar')) {
                $thumbnilsize = 'cynic-blog-thumbnail-with-slidebar';
            } else {
                $thumbnilsize = 'cynic-blog-thumbnail-without-slidebar';
            }
        } else {
