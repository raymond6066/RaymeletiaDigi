...
if ( class_exists( 'WP_Customize_Control' ) ) {
	 ...
	 class Skyrocket_Image_Checkbox_Custom_Control extends WP_Customize_Control {
 		 ...
 		 */
  		public function enqueue() {
 			wp_enqueue_style( 'skyrocket-custom-controls-css', trailingslashit( get_template_directory_uri() ) . 'lib/functions/trendy-agency/customizer-custom-controls/css/customizer.css', array(), '1.0', 'all' );
  		}
 		/**
	...
	class Skyrocket_Single_Accordion_Custom_Control extends WP_Customize_Control {
		 ...
		 */
		public function enqueue() {
			wp_enqueue_script( 'skyrocket-custom-controls-js', trailingslashit( get_template_directory_uri() ) . 'lib/functions/trendy-agency/customizer-custom-controls/js/customizer.js', array( 'jquery' ), '1.0', true );
			wp_enqueue_style( 'skyrocket-custom-controls-css', trailingslashit( get_template_directory_uri() ) . 'lib/functions/trendy-agency/customizer-custom-controls/css/customizer.css', array(), '1.0', 'all' );
	...
	class Skyrocket_Slider_Custom_Control extends WP_Customize_Control {
		 ...
		 */
		public function enqueue() {
			wp_enqueue_script( 'skyrocket-custom-controls-js', trailingslashit( get_template_directory_uri() ) . 'lib/functions/illustration/customizer-custom-controls/js/customizer.js', array( 'jquery', 'jquery-ui-core' ), '1.0', true );
			wp_enqueue_style( 'skyrocket-custom-controls-css', trailingslashit( get_template_directory_uri() ) . 'lib/functions/illustration/customizer-custom-controls/css/customizer.css', array(), '1.0', 'all' );
		}
		/**
		 * Render the control in the customizer
		 */
		...
		?>
			<div class="slider-custom-control">
				<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span><input type="text" id="<?php echo esc_attr( $this->id ); ?>" name="<?php echo esc_attr( $this->id ); ?>" value="<?php echo esc_attr( $this->value() ); ?>" class="customize-control-slider-value" <?php $this->link(); ?> />
	...
	class Skyrocket_Dropdown_Select2_Custom_Control extends WP_Customize_Control {
		...
		public function enqueue() {
			...
			wp_enqueue_script( 'skyrocket-custom-controls-js', trailingslashit( get_template_directory_uri() ) . 'lib/functions/illustration/customizer-custom-controls/js/customizer.js', array( 'skyrocket-select2-js' ), '1.0', true );
			wp_enqueue_style( 'skyrocket-custom-controls-css', trailingslashit( get_template_directory_uri() ) . 'lib/functions/illustration/customizer-custom-controls/css/customizer.css', array(), '1.1', 'all' );
	...
	class Skyrocket_TinyMCE_Custom_control extends WP_Customize_Control {
		 ...
		 */
		public function enqueue(){
			wp_enqueue_script( 'skyrocket-custom-controls-js', trailingslashit( get_template_directory_uri() ) . 'lib/functions/illustration/customizer-custom-controls/js/customizer.js', array( 'jquery' ), '1.0', true );
			wp_enqueue_style( 'skyrocket-custom-controls-css', trailingslashit( get_template_directory_uri() ) . 'lib/functions/illustration/customizer-custom-controls/css/customizer.css', array(), '1.0', 'all' );
			wp_enqueue_editor();
		}
		/**
	...
	class Skyrocket_Google_Font_Select_Custom_Control extends WP_Customize_Control {
		...
		public function skyrocket_getGoogleFonts( $count = 30 ) {
			...
			$fontFile = trailingslashit( get_template_directory_uri() ) . 'lib/functions/illustration/customizer-custom-controls/inc/google-fonts-alphabetical.json';
			if ( $this->fontOrderBy === 'popular' ) {
				$fontFile = trailingslashit( get_template_directory_uri() ) . 'lib/functions/illustration/customizer-custom-controls/inc/google-fonts-popularity.json';
			}
	...
	class Skyrocket_Customize_Alpha_Color_Control extends WP_Customize_Control {
		 ...
		 */
		public function enqueue() {
			wp_enqueue_script( 'skyrocket-custom-controls-js', trailingslashit( get_template_directory_uri() ) . 'lib/functions/illustration/customizer-custom-controls/js/customizer.js', array( 'jquery', 'wp-color-picker' ), '1.0', true );
			wp_enqueue_style( 'skyrocket-custom-controls-css', trailingslashit( get_template_directory_uri() ) . 'lib/functions/illustration/customizer-custom-controls/css/customizer.css', array( 'wp-color-picker' ), '1.0', 'all' );
		}
		/**
		 * Render the control in the customizer
		 */
     ...
     * @return integer	Returned integer value
     */
    if ( ! function_exists( 'skyrocket_sanitize_font_weight' ) ) {
        function skyrocket_sanitize_allinone( $input ) {
            return (int) $input;
        }
    }

    class Skyrocket_FontWeight_Custom_Control extends WP_Customize_Control {
        /**
         * The type of control being rendered
         */
        public $type = 'all_in_one_control';
        ...
        private $font_size, $font_size_in, $line_height,$font_attribute, $font_weight;
        /**
         * Get our list of fonts from the json file
         */
        /**
        ...
        public function __construct( $manager, $id, $args = array(), $options = array() ) {
            ...
            $values = $this->value();
            $json_array = json_decode($values, true);
            $fontsArray = getCynicOptionsVal('body-font');
            $fonts = json_decode($fontsArray, true);
            $font_family = (isset($fonts['font']) && !empty($font['font'])) ? $font['font'] : "Fira Sans";

            $this->font_size = (isset($json_array['font_size'])) ? $json_array['font_size'] : "";
            $this->font_size_in = (isset($json_array['font_size_in'])) ? $json_array['font_size_in'] : "";
            $this->line_height = (isset($json_array['line_height'])) ? $json_array['line_height'] : "";

            $this->font_attribute = ($fontsArray && !empty($fontsArray)) ? $fontsArray : $this->value();
            ...
            $this->fontList = $this->_all_skyrocket_getGoogleFonts( 'all' );

            // Decode the default json font value
            $this->fontValues = json_decode( $this->value() );
            // Find the index of our default font within our list of Google fonts
            $this->fontListIndex = $this->_all_skyrocket_getFontIndex( $this->fontList, $font_family );
        }
        /**
         * Enqueue our scripts and styles
         */
        public function enqueue() {
            wp_enqueue_script( 'skyrocket-select2-js', 'https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.6-rc.0/js/select2.min.js', array( 'jquery' ), '4.0.6', true );
            wp_enqueue_script( 'skyrocket-custom-controls-js', trailingslashit( get_template_directory_uri() ) . 'lib/functions/illustration/customizer-custom-controls/js/customizer.js', array( 'skyrocket-select2-js' ), CYNIC_THEME_VERSION, true );
            wp_enqueue_style( 'skyrocket-custom-controls-css', trailingslashit( get_template_directory_uri() ) . 'lib/functions/illustration/customizer-custom-controls/css/customizer.css', array(), CYNIC_THEME_VERSION, 'all' );
            wp_enqueue_style( 'skyrocket-select2-css', 'https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.6-rc.0/css/select2.min.css', array(), '4.0.6', 'all' );
        }
        ...
        public function render_content() {
            ...
            $isFontInList = false;
            $fontListStr = ''; ?>
            <div class="font-weight-custom-control text_radio_button_control <?php echo esc_attr( $this->id ); ?>">
                    <?php if( !empty( $this->label ) ) { ?>
                        <span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
                    <?php } ?>
                    <?php if( !empty( $this->description ) ) { ?>
                    ...
                    <input type="text" id="<?php echo esc_attr( $this->id ); ?>"
                        ...
                        <?php $this->link() ?> class="<?php echo esc_attr( $this->id ); ?>">

                    <?php if( !empty($this->fontList) ) { ?>
                        <div class="customize-control-description">Select Font Weight</div>
                            <div class="weight-style">
                                <select class="google-fonts-boldweight-style"
                                name="font_weight" data-class="<?php echo esc_attr( $this->id ); ?>">
                                    <?php
                                    $optionCount = 0;
                                    foreach( $this->fontList[$this->fontListIndex]->variants as $key => $value ) {
                                        // Only add options that aren't italic
                                        if( strpos( $value, 'italic' ) === false ) {
                                            echo '<option value="' . $value . '" ' . selected( $this->value(), $value, false ) . '>' . $value . '</option>';
                                            $optionCount++;
                                        }
                                    }
		 ...
		 * Find the index of the saved font in our multidimensional array of Google Fonts
		 */
		public function _all_skyrocket_getFontIndex( $haystack, $needle ) {
			foreach( $haystack as $key => $value ) {
				if( $value->family == $needle ) {
					return $key;
				}
		 ...
		 * Return the list of Google Fonts from our json file. Unless otherwise specfied, list will be limited to 30 fonts.
		 */
		public function _all_skyrocket_getGoogleFonts( $count = 30 ) {
			// Google Fonts json generated from https://www.googleapis.com/webfonts/v1/webfonts?sort=popularity&key=YOUR-API-KEY
			$fontFile = trailingslashit( get_template_directory_uri() ) . 'lib/functions/illustration/customizer-custom-controls/inc/google-fonts-alphabetical.json';
			if ( $this->fontOrderBy === 'popular' ) {
				$fontFile = trailingslashit( get_template_directory_uri() ) . 'lib/functions/illustration/customizer-custom-controls/inc/google-fonts-popularity.json';
