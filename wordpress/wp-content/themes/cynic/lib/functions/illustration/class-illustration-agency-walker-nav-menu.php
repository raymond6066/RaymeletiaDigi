 ...
 * @see Walker
 */
class Illustration_Agency_Walker_Nav_Menu extends Walker_Nav_Menu
{
    ...
    public function start_lvl(&$output, $depth = 0, $args = array())
    {
        ...
        // Default class.
        $classes = array('submenu custom-dropdown-menu');
    ...
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        if (in_array('menu-item-has-children', $classes)) {
            ...
            $atts_classs = 'nav-link';
        }
        if (in_array('cynic-nav-menu-button', $classes)) {
            $atts_classs .= 'custom-btn btn-mid';
        }
        ...
        $atts = array();
        $atts['class'] = '' . $atts_classs;

        if (get_theme_mod('cynic_layouts') == 2) {
            $atts['class'] = 'page-scroll nav-link' . $atts_classs;
        }
        ...
        if (get_theme_mod('cynic_layouts') == 2 && !is_front_page() && $item->type_label == "Custom Link") {
            $atts['href'] = !empty($item->url) ? esc_url(home_url('/')) . $item->url : '';
        } else {
                ...
                if (get_theme_mod('cynic_header_button_display')) {
                    ...
                    if (get_theme_mod('cynic_is_header_button_open_with_modal') == "bookmark") {
                        ...
                        if ($pageID) {
                            ...
                            return '<li class="nav-item"><a class="custom-btn btn-small" href="' . esc_url($pageLink) . '" >' . esc_html($button_text) . '</a></li>';
                        }
                    } else if (get_theme_mod('cynic_is_header_button_open_with_modal') == "modal") {
                        $pageID = get_theme_mod('cynic_header_button_page');
                        if ($pageID) {
                            return '<li class="nav-item"><a class="custom-btn btn-small" href="javascript:void(0)" data-toggle="modal" data-target="#get-a-quote-modal">' . esc_html($button_text) . '</a></li>';
                        }
                    } else {
                        return '<li class="nav-item"><a class="custom-btn btn-small" href="javascript:void(0)">' . $button_text . '</a></li>';
                    }
                }
            }
        }
