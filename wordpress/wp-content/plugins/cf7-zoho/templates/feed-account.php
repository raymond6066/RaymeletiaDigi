...
.vxcf_options_row .howto{
  ...
  ?>
  <div class="vx_wrap"> 
    <h2 class="vx_img_head"><img alt="<?php esc_html_e("Zoho Feeds", 'contact-form-zoho-crm') ?>" title="<?php esc_html_e("Zoho Feeds", 'contact-form-zoho-crm') ?>" src="<?php echo $this->get_base_url()?>images/zoho-crm-logo.png?ver=1" /> <?php 
echo sprintf(esc_html__('Edit Zoho Feed # %d','contact-form-zoho-crm') ,$fid); 
  if(empty($id) || $new_account_id != $id){
 ?> <a href="<?php echo esc_url($new_feed_link) ?>" class="add-new-h2"><?php esc_html_e("Add New Feed",'contact-form-zoho-crm'); ?></a> 
 <?php
}
 ?>
 <a href="<?php echo esc_url($feeds_link) ?>" class="add-new-h2"><?php esc_html_e("Back to Feeds",'contact-form-zoho-crm'); ?></a>
...
if(empty($feed_name)){
  ...
  ?>
  <div class="error" id="message" style="margin-top:20px;"><?php echo wpautop(sprintf(esc_html__("We are unable to login to Zoho with the provided API token. Please make sure they are valid in the %sSettings Page%s", 'contact-form-zoho-crm'), "<a href='".esc_url($this->link_to_settings())."'>", "</a>")); ?></div>
  <?php
  // return;
  } 
  ?>
  ...
  <?php wp_nonce_field("vx_nonce") ?>
    <div class="vx_div">
      <div class="vx_head">
<div class="crm_head_div"> <?php esc_html_e('1. Enter feed name',  'contact-form-zoho-crm' ); ?></div>
...
<div class="crm_clear"></div> 
  ...
  <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_name" class="left_header"><?php esc_html_e("Feed Name", 'contact-form-zoho-crm'); ?>
  ...
  <div class="vx_div">
        <div class="vx_head">
<div class="crm_head_div"> <?php esc_html_e('2. Select Zoho Account.', 'contact-form-zoho-crm'); ?></div>
<div class="crm_btn_div" title="<?php esc_html_e('Expand / Collapse','contact-form-zoho-crm') ?>"><i class="fa crm_toggle_btn vx_action_btn fa-minus"></i></div>
<div class="crm_clear"></div> 
  </div>
  ...
  <div class="vx_group">
    <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_account" class="left_header"><?php esc_html_e("Zoho Account", 'contact-form-zoho-crm'); ?>
  <?php $this->tooltip("vx_sel_object") ?>
 </label>
  </div>
  <div class="vx_col2">
  <select id="vx_account" class="crm_sel load_object" name="account" autocomplete="off">
  <option value=""><?php esc_html_e("Select a Zoho Account", 'contact-form-zoho-crm'); ?></option>
  <?php
  ...
  <div class="clear"></div>
    ...
    <div class="vx_div">
        <div class="vx_head">
<div class="crm_head_div"> <?php esc_html_e('3. Select Contact Form.', 'contact-form-zoho-crm'); ?></div>
<div class="crm_btn_div" title="<?php esc_html_e('Expand / Collapse','contact-form-zoho-crm') ?>"><i class="fa crm_toggle_btn vx_action_btn fa-minus"></i></div>
<div class="crm_clear"></div> 
  </div>
  ...
  <div class="vx_group">
    <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_form_id" class="left_header"><?php esc_html_e("Contact Form Form", 'contact-form-zoho-crm'); ?>
  <?php $this->tooltip("vx_sel_form") ?>
 </label>
  </div>
  <div class="vx_col2">
  <select id="vx_form_id" class="crm_sel load_object" name="form_id" autocomplete="off">
  <option value=""><?php esc_html_e("Select a Form", 'contact-form-zoho-crm'); ?></option>
  <?php
 ...
 echo "<option value='".esc_attr($form_id_arr)."' $sel>".esc_html($form_title)."</option>"; 
  ...
  <div class="clear"></div>
    ...
    <div id="crm_ajax_div_object" style="display:none; text-align: center; line-height: 100px;"><i> <?php esc_html_e('Loading, Please Wait...','contact-form-zoho-crm'); ?></i></div>
...
//post validation
 ...
 if($(".vx_required").length){
   ...
   var value=""; 
    ...
    attach_error.append('<div class="entry_row vx_entry_error"><div class="vx_error"><i class="fa fa-warning"></i> <?php esc_html_e('This is a required field','contact-form-zoho-crm') ?></div></div>');
    return false;
    }  
   })  
 }
  ...
  e.preventDefault(); 
    if(!confirm('<?php esc_html_e('Are you sure to remove ?','contact-form-zoho-crm') ?>')){
      return;
  }
...
if(type){ row.find('.vxc_field_option').hide(); }
  ...
  if(field_name_select.length){
    ...
    if(field_name == '' || crm_fields[field_name] == ''){
     alert('<?php esc_html_e('Please Select Field Name','contact-form-zoho-crm') ?>');
     return;   
    }  
...
if(type){ row.find('.vxc_field_option').hide(); }
  ...
  $(document).on('click','.vxc_tips',function(e){
    ...
    jQuery('#crm_note_fields').select2({ placeholder: '<?php esc_html_e('Select Field','contact-form-zoho-crm') ?>'});
} 
...
vx_select();
  function vx_select(){
      $('#vx_add_fields_select').select2({ placeholder: '<?php esc_html_e('Select Field','contact-form-zoho-crm') ?>'});
      add_note_sel();
  }
